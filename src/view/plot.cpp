...
void Plot::render_graph()
{
    ...
    for (int  i = 0; i < N; i++) {
        ...
        api_calculate_str_with_x(math_expression, (long double)(X_temp+i*h_step), &x_calc);
...
{
    if (is_set && ui->xmin_val->text().toDouble() < ui->xmax_val->text().toDouble()) {
        x_min = ui->xmin_val->text().toDouble();
        ui->plotRender->clearGraphs();
        render_graph();
    }
...
{
    if (is_set && ui->ymin_val->text().toDouble() < ui->ymax_val->text().toDouble()) {
        y_min = ui->ymin_val->text().toDouble();
        ui->plotRender->clearGraphs();
        render_graph();
    }

...
{
    if (ui->deposit_amount_val->text().isEmpty() || ui->term_val->text().isEmpty() || ui->term_val->text().isEmpty() ||
        ui->interest_rate_val->text().isEmpty() || (!ui->months->isChecked() && !ui->years->isChecked()) ||
        ...
        long double total_earned = get_total_earned(&deposit_amount, term, interest_rate, mode, is_capitalization, monthly_replenishment, monthly_withdrawal);
        long double tax_amount = get_tax_amount(total_earned, tax_rate);
        long double total_amount = get_total_amount(deposit_amount, total_earned, tax_amount);

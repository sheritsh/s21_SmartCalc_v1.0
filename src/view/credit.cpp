...
{
    if (ui->amount_val->text().isEmpty() || ui->term_val->text().isEmpty() || ui->interest_rate->text().isEmpty() || (!ui->months->isChecked() && !ui->years->isChecked()) || (!ui->annuity->isChecked() && !ui->diff->isChecked())) {
        // error window
        QMessageBox::warning(this, "Not all filled", "Please fill in all fields to enter");
    } else {
        ...
        if (ui->annuity->isChecked()) {
            long double monthly_payment = get_monthly_payment_annuity(loan_amount, interest_rate, term);
            long double total_payment = get_total_payment_annuity(monthly_payment, term);
            long double overpayment = get_overpayment_on_credit(loan_amount, total_payment);
            ...
            long double total_payment = get_total_payment_diff(loan_amount, term, interest_rate);
            long double monthly_payment = get_monthly_payment_diff(total_payment, term);
            long double overpayment = get_overpayment_on_credit(loan_amount, total_payment);
            QString mp = QString::number(monthly_payment, 'g', 7);
            QString tp = QString::number(total_payment, 'g', 7);
            QString ovp = QString::number(overpayment, 'g', 7);

...
MainWindow::MainWindow(QWidget *parent)
    ...
    input_area_validator(QRegularExpression("^[(sin|cos|ln|tan|log|mod|asin|atan|acos|sqrt|)0-9^/*-+xe.]{255}"))
{
    ui->setupUi(this);
    // make custom window
...
void MainWindow::mouseMoveEvent(QMouseEvent *event) {
    ...
    plotWindow->move(plotWindow->x() + new_pos.x(), plotWindow->y() + new_pos.y());
    depositWindow->move(depositWindow->x() + new_pos.x(), depositWindow->y() + new_pos.y());
    creditWindow->move(creditWindow->x() + new_pos.x(), creditWindow->y() + new_pos.y());
    ...
    if (ui->pushButton_eq->text() == "=") {
        ...
        if (ui->label_fxico->isVisible()) {
            if (check_is_x_set()) {
                // API to count with X
                long double res = 0;
                is_error = api_calculate_str_with_x(cstr, (long double)ui->lineedit_xinput->text().toDouble(), &res);
                if (!is_error) {
                ...
                QMessageBox::warning(this, "X not defined", "Please enter value ​​for X");
            }
        } else {
...
void MainWindow::bind_slots()
{
    ...
    connect(ui->InputArea, SIGNAL(textChanged(QString)), this, SLOT(x_input_handler()));
    // |- navigation
    connect(ui->pushButton_close, SIGNAL(clicked()), this, SLOT(close()));
    connect(ui->pushButton_minimize, SIGNAL(clicked()), this, SLOT(minimization()));
    connect(ui->pushButton_plot, SIGNAL(clicked()), this, SLOT(open_windows()));
    connect(ui->pushButton_deposit, SIGNAL(clicked()), this, SLOT(open_windows()));
    connect(ui->pushButton_credit, SIGNAL(clicked()), this, SLOT(open_windows()));
    // |- input buttons
    // |-- numbers
    connect(ui->pushButton_one, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_two, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_three, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_four, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_five, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_six, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_seven, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_eight, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_nine, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_zero, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    ...
    connect(ui->pushButton_pow, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_sqrt, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_opbracket, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    connect(ui->pushButton_clbracket, SIGNAL(clicked()), this, SLOT(input_btns_clicked()));
    // |-- functions
...
void MainWindow::on_InputArea_textChanged(const QString &arg1)
{
    if (arg1.length() == 2) {
        if (arg1[0] == '0'){
            if (arg1[1] != '+' && arg1[1] != '-' && arg1[1] != '/' && arg1[1] != '*' && arg1[1] != '.' && arg1[1] != '^') {
...
{
    if (ui->pushButton_plot->isChecked() || ui->pushButton_deposit->isChecked() || ui->pushButton_credit->isChecked() || ui->pushButton_rpn->isChecked()) {
        plotWindow->hide();
        ui->fx->hide();
        ui->pushButton_eq->setText("=");
        depositWindow->hide();
